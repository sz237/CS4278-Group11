        ...
        {loading ? (
          ...
          <>
            ...
            <div className="d-flex align-items-start gap-4 mb-4">
              {/* avatar placeholder */}
              <div style={{ width: 110, height: 110, borderRadius: "50%", border: "6px solid #000", background: "#efefef" }} />
              <div>
                <h2 className="h4 mb-1">{profile?.firstName} {profile?.lastName}</h2>
                <div className="text-muted small">
                  {posts.length} posts<br />
                  {profile?.connectionsCount ?? 0} connections<br />
                </div>
                <Link to="/edit-profile" className="d-inline-flex align-items-center gap-2 mt-2 text-decoration-none" style={{ color: "#000" }}>
                ...
                <div key={p.id} className="col-12 col-md-4">
                  <div className="p-2 border border-2" style={{ borderColor: "#000" }}>
                    ...
                    <div className="d-flex gap-2 mt-2">
                      {p.imageUrl ? (
                        <img src={p.imageUrl} alt={p.title} style={{ width: 90, height: 60, objectFit: "cover", border: "1px solid #000" }} />
                      ) : null}
                      <p className="small mb-0">{p.description}</p>
                    </div>

                    <div className="d-flex justify-content-end align-items-center gap-3 mt-2">
                      <button className="btn btn-link p-0" onClick={() => onEdit(p)} title="Edit">
                        <span style={{ fontSize: 20, color: "#000" }}>🖉</span>
                      </button>
                      <button className="btn btn-link p-0" onClick={() => onDelete(p.id)} title="Delete">
